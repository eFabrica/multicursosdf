...
if($_REQUEST["act"] == "salvar"){
	...
	if($_ClassMensagens->getMensagem_erro() == ""){
		...
		$_ClassRn->getDadosTable("materias", "id", "unidade = '" . $_dadosUnidade->id . "' AND materia = '" . $_REQUEST["materia"] . "' AND curso = '"  . $_REQUEST["curso"] . "' AND deletado = 'N'");
	...
	if($_ClassMensagens->getMensagem_erro() == ""){
		...
		// Cadastra materia
		$cadastraMateria = $_ClassMysql->query("INSERT INTO `materias` SET unidade = '" . $_dadosUnidade->id . "',
		...
		if($cadastraMateria){
			...
			$_ClassMensagens->setMensagem_sucesso("Matéria gravada com sucesso!<br><br>[ <a href='?sessao=materias&ref=novo'>Atualizar</a> ]");
			...
			<form action="" method="POST" name="formMateria">
				...
				<table width="99%" border="0" cellpadding="2" cellspacing="2" align="center">
						...
						<td width='90%' align='left'>
							<select name="curso">
								...
								while($trazCursos = mysql_fetch_object($buscaCursos)){
									...
									?>
									<option value="<?php print($trazCursos->id);?>" <?php print((($_REQUEST["curso"] == $trazCursos->id)?"selected":""));?>><?php print($trazCursos->nome);?> (<?php print($trazCursos->sigla);?>)</option>
									<?php
					...
					<tr>
						...
						<td align='left'><input type="text" name="sigla" size="5" value="<?php echo $_REQUEST["sigla"]?>"></td>
					</tr>
					<tr>
						<td align="right"><b><font class="obrig">(*)</font> Matéria:</b></td>
						<td align='left'><input type="text" name="materia" size="25" value="<?php echo $_REQUEST["materia"]?>"></td>
					</tr>
					<tr>
						<td align="right"><b><font class="obrig">(*)</font> Carga Horária:</b></td>
						<td align='left'><input type="text" name="cargahoraria" size="15" value="<?php echo $_REQUEST["cargahoraria"]?>"></td>
					</tr>
					<tr>
						<td align="right" valign="top"><b>Descrição:</b></td>
						<td align='left'>
							<?php
							// Importa FCK Editor
							...
							$oFCKeditor->Value =  $_REQUEST["descricao"];
