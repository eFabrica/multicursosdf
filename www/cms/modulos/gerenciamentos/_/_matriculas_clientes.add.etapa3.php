...
if($_SESSION["matricula"]["idAluno"] > 0){
	...
	if($_REQUEST["act"] == "salvar"){
		...
		if($_ClassMensagens->getMensagem_erro() == ""){
			...
			// Redireciona
			print($_ClassUtilitarios->redirecionarJS("?sessao=matriculas&subsessao=" . $_REQUEST["subsessao"] . "&ref=novo&etapa=4"));
						...
						<td width='85%' align='left'>
							<select name="empresa">
								...
								while($trazEmpresas = mysql_fetch_object($buscaEmpresas)){
									...
									?>
									<option value="<?php print($trazEmpresas->id);?>" <?php print((((($_REQUEST["empresa"] != "")?$_REQUEST["empresa"]:$_SESSION["matricula"]["idEmpresa"]) == $trazEmpresas->id)?"selected":""));?>><?php print($trazEmpresas->razaosocial);?></option>
									<?php
