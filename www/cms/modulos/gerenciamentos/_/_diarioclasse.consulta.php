...
if($_REQUEST["ref"] == ""){
	...
	<tr>
		<td class="table_main">
			<form action="?sessao=diarioclasse&ref=buscar" method="POST" name="formDiarioClasse">
				...
				<table border="0" cellpadding="2" cellspacing="2" align="center">
					...
					<tr>
						<td align="right" valign="top"><strong>Conteúdo:</strong></td>
					  	<td align="left"><textarea rows="1" cols="60" name="conteudo" onfocus="this.rows = 5;" onblur="this.rows = 1;" disabled></textarea></td>
					  	<td align="center"><input type="checkbox" name="habilitarConteudo" value="sim" onClick="disen(document.formDiarioClasse.conteudo);" ></td>
					</tr>
					<tr>
						<td align="left">&nbsp;</td>
						...
						<td align="center"><input type="checkbox" name="todas" value="sim" onClick=" disen(document.formDiarioClasse.habilitarData);checkedDisable(document.formDiarioClasse.habilitarData);
																									 ...
																									 disen(document.formDiarioClasse.habilitarConteudo);checkedDisable(document.formDiarioClasse.habilitarConteudo);
																									 ...
																									 disable(document.formDiarioClasse.conteudo);
	...
	if($_REQUEST["act"] == "deletar"){
		...
		for($y = 0; $y < count($_REQUEST["registros"]); $y++){
			...
			// Deleta Grades
			$_ClassMysql->query("UPDATE `diarioclasse` SET deletado = 'S', quemdeletou = '" . $_dadosLogado->id . "', datahorad = now() WHERE id = '" . $_REQUEST["registros"][$y] . "'");
		...
		$_ClassMensagens->setMensagem_sucesso("<b>" . count($_REQUEST["registros"]) . "</b> Diário(s) de Classe foi(ram) deletado(s) com sucesso!<br><br>[ <a href='?" . str_replace("&act=cancelar", "", $_SERVER['QUERY_STRING']) . "'>Atualizar</a> ]");
			...
			<table width="99%" border="0" cellpadding="2" cellspacing="2">
				<tr>
					...
					<td width='85%' align='left'>
						<select onchange="if(confirm('Deseja mesmo deletar este(s) registro(s)?')){document.formDiarioClasse.act.value = '' + this.options[this.selectedIndex].value; document.formDiarioClasse.submit();}">
							<option value=""></option>
					...
					<td colspan="2">
						<form action="" method="POST" name="formDiarioClasse">
							...
							<table class="consulta" cellspacing="1" align="center">
								<thead>
									<tr>
										<th width="1%"><?php print($_ClassUtilitarios->OrdemControl("#", "?sessao=diarioclasse&ref=buscar&pg=" . $_REQUEST["pg"], "id", $pathInc)); ?></th>
										<th width="1%" align="center"><input type="checkbox" onclick="select_all('formDiarioClasse', 'registros[]')"></th>
										<th width="15%"><?php print($_ClassUtilitarios->OrdemControl("Data", "?sessao=diarioclasse&ref=buscar&pg=" . $_REQUEST["pg"], "data", $pathInc)); ?></th>
										<th width="15%"><?php print($_ClassUtilitarios->OrdemControl("Turma", "?sessao=diarioclasse&ref=buscar&pg=" . $_REQUEST["pg"], "turma", $pathInc)); ?></th>
										<th width="15%"><?php print($_ClassUtilitarios->OrdemControl("Turno", "?sessao=diarioclasse&ref=buscar&pg=" . $_REQUEST["pg"], "turno", $pathInc)); ?></th>
										<th width="15%"><?php print($_ClassUtilitarios->OrdemControl("Matéria", "?sessao=diarioclasse&ref=buscar&pg=" . $_REQUEST["pg"], "materia", $pathInc)); ?></th>
										<th width="30%"><?php print($_ClassUtilitarios->OrdemControl("Instrutor", "?sessao=diarioclasse&ref=buscar&pg=" . $_REQUEST["pg"], "instrutor", $pathInc)); ?></th>
										<th width="5%"><?php print($_ClassUtilitarios->OrdemControl("Sala", "?sessao=diarioclasse&ref=buscar&pg=" . $_REQUEST["pg"], "sala", $pathInc)); ?></th>
										<th width="5%">Funções</th>
									</tr>
								</thead>
								<tbody>
									<?php	
									...
									$_ClassPaginacao->setUrl("?sessao=diarioclasse&ref=buscar&ordem=" . $_REQUEST["ordem"] . "&campo=" . $_REQUEST["campo"] . "&filtro=" . $_REQUEST["filtro"]);
									...
									if($_ClassPaginacao->getTotalAchadoQuery() == 0){
										...
										while($trazResultados = mysql_fetch_object($_ClassPaginacao->getBusca())){
											...
											<tr class=row0>
												...
												<td align="center"><?php print($dadosCurso->sigla);?><?php print($dadosTurma->numero . " <b>(" . $_ClassData->transformaData($dadosTurma->datainicio,2 ) . " à " . $_ClassData->transformaData($dadosTurma->datatermino,2 ) . ")</b>");?> <img src="<?php print($pathInc . "imagens/icones/" . $dadosTurma->concluido . "M.png");?>"></td>
												...
												<td align="center"><a href="?sessao=diarioclasse<?="&ordem=" . $_REQUEST['ordem'] . "&campo=" . $_REQUEST['campo'] . "&pg=" . $_REQUEST['pg'] . "&ref=edit&idRegistro=" . $trazResultados->id?>"><img src="<?php print($pathInc);?>imagens/icones/edit.gif" border="0" alt="Editar" title="Editar"></a>&nbsp;</td>
											</tr>
											<?php
										}
