...
if($_SESSION["matricula"]["idAluno"] > 0){
	...
	if($_SESSION["matricula"]["idTurma"] > 0){
		
		// Dados do Aluno
		$dadosAluno = $_ClassRn->getDadosTable("alunos", "numerorg, academiaform", "id = '" . $_SESSION["matricula"]["idAluno"] . "'");
		
		// Dados da Turma
		$dadosTurma = $_ClassRn->getDadosTable("turmas", "*", "id = '" . $_SESSION["matricula"]["idTurma"] . "' AND concluido = 'N' AND deletado = 'N'");
		...
		if($dadosTurma->vagasocupadas < $dadosTurma->vagas){
			...
			$buscaMatriculas = $_ClassMysql->query("SELECT aluno FROM `matriculas` WHERE unidade = '" . $_dadosUnidade->id . "' AND turma = '" . $_SESSION["matricula"]["idTurma"] . "' AND concluido = 'N' AND deletado = 'N'");
			...
			while($trazMatriculas = mysql_fetch_object($buscaMatriculas)){
				...
				if($dadosAluno->cpf == $dadosAlunoMat->cpf && $permissao == true){
					...
					// Redieciona
					print($_ClassUtilitarios->redirecionarJS("?sessao=matriculas&ref=novo&etapa=4", 1, array("Este aluno já está nesta turma.")));
			...
			if($permissao){
				...
				<tr>
					<td align='left'>
						...
						// Seta Query
						$_ClassConsulta->setQuery("SELECT * FROM `turmas` WHERE id = '" . $_SESSION["matricula"]["idTurma"] . "' AND concluido = 'N' AND deletado = 'N' ORDER BY id ASC");
				...
				if($_REQUEST["act"] == "salvar"){
					...
					// Verifica campos
					$_ClassMensagens->setMensagem_erro($_ClassForm->cb($_REQUEST["numero"], "É necessário informar o número da matrícula."));
					
					// Verifica se tem erro
					if($_ClassMensagens->getMensagem_erro() == ""){
						
						// Verifica se só contém número
						if(!ctype_digit($_REQUEST["numero"])){
						...
						if(count($_REQUEST["formapagamento"]) == 0){
							...
							for($i =0; $i < count($_REQUEST["formapagamento"]); $i++){
								...
								if($_ClassMensagens->getMensagem_erro() == ""){
									...
									$buscaNumeroMatricula = $_ClassMysql->query("SELECT id FROM `matriculas` WHERE unidade = '" . $_dadosUnidade->id . "' AND numero = '" . $_REQUEST["numero"] . "' AND deletado = 'N'");
									
									// Verifica o total achado 
									if(mysql_num_rows($buscaNumeroMatricula) > 0){
										
										// Redieciona
										print($_ClassUtilitarios->redirecionarJS("?sessao=matriculas&ref=novo&etapa=5", 1, array("O Número desta matrícula já está sendo usado. Por Favor, escolha outro.")));
								...
								if($_ClassMensagens->getMensagem_erro() == ""){
									...
									$cadastraMatricula = $_ClassMysql->query("INSERT INTO `matriculas` SET unidade = '" . $_dadosUnidade->id . "',
																										   ...
																										   turma = '" . $_SESSION["matricula"]["idTurma"] . "',"
																										   ...
																										   . ((is_array($_REQUEST["formapagamento"]) && array_search("dinheiro", $_REQUEST["formapagamento"]) !== false)?"pg_dinheiro = 'S', valor_dinheiro = '" . $_ClassDinheiro->limpaFormatacaoMoeda($_REQUEST["vl_dinheiro"]) . "', ":"pg_dinheiro = 'N', ")
																										   . ((is_array($_REQUEST["formapagamento"]) && array_search("ch", $_REQUEST["formapagamento"]) !== false)?"pg_cheque = 'S', cheque_parcelas = '" . $_REQUEST["parcelas_ch"] . "', valor_cheque = '" . $_ClassDinheiro->limpaFormatacaoMoeda($_REQUEST["vl_ch"]) . "', ":"pg_cheque = 'N', ")
																										   . ((is_array($_REQUEST["formapagamento"]) && array_search("cc", $_REQUEST["formapagamento"]) !== false)?"pg_cartaocredito = 'S', valor_cartaocredito = '" . $_ClassDinheiro->limpaFormatacaoMoeda($_REQUEST["vl_cc"]) . "', ":"pg_cartaocredito = 'N', ")
																										   . ((is_array($_REQUEST["formapagamento"]) && array_search("cd", $_REQUEST["formapagamento"]) !== false)?"pg_cartaodebito = 'S', valor_cartaodebito = '" . $_ClassDinheiro->limpaFormatacaoMoeda($_REQUEST["vl_cd"]) . "', ":"pg_cartaodebito = 'N', ")
																										   . ((is_array($_REQUEST["formapagamento"]) && array_search("bb", $_REQUEST["formapagamento"]) !== false)?"pg_boleto = 'S', boleto_parcelas = '" . $_REQUEST["parcelas_bb"] . "', valor_boleto = '" . $_ClassDinheiro->limpaFormatacaoMoeda($_REQUEST["vl_bb"]) . "', ":"pg_boleto = 'N', ")
									...
									if($cadastraMatricula){
										...
										# Parcelas
											...
											if(is_array($_REQUEST["formapagamento"]) && array_search("bb", $_REQUEST["formapagamento"]) !== false){
												...
												for($p = 1; $p <= $_REQUEST["parcelas_bb"]; $p++){
													...
													// Cadastra Parcelas
													$cadastraParcelasBB = $_ClassMysql->query("INSERT INTO `parcelas` SET matricula = '" . $idMatricula . "',
																														  numero = '" . $p . "',
																														  tipo = 'B',
											...
											if(is_array($_REQUEST["formapagamento"]) && array_search("ch", $_REQUEST["formapagamento"]) !== false){
												...
												for($p = 1; $p <= $_REQUEST["parcelas_ch"]; $p++){
													...
													// Cadastra Parcelas
													$cadastraParcelasCH = $_ClassMysql->query("INSERT INTO `parcelas` SET matricula = '" . $idMatricula . "',
																														  numero = '" . $p . "',
																														  tipo = 'C',
										...
										# Sucesso
											...
											// Ocupa Vaga
											$ocupaVaga = $_ClassMysql->query("UPDATE `turmas` SET vagasocupadas = (vagasocupadas+1) WHERE id= '" . $_SESSION["matricula"]["idTurma"] . "'");
										
											// Cadastra na sessão o id da Matrícula
											$_SESSION["matricula"]["idMatricula"] = $idMatricula;
											
											// Redireciona
											print($_ClassUtilitarios->redirecionarJS("?sessao=matriculas&ref=novo&etapa=6"));
				...
				// Dados da Matrícula
				$dadosMatricula = $_ClassRn->getDadosTable("matriculas", "numero", "unidade = '" . $_dadosUnidade->id . "' AND deletado = 'N' ORDER BY numero DESC LIMIT 0, 1");
				?>
				<tr>
					<td style='height:5px';>&nbsp;</td>
				</tr>
						...
						<form action="#form" method="POST" name="formMatricula">
							...
							<table width="100%" border="0" cellpadding="2" cellspacing="2" align="left">
								<tr>
									...
									<td width="85%" colspan="2" align="left"><input type="text" name="numero" size="5" value="<?php print($_REQUEST["numero"]);?>""></td>
								</tr>
								<tr>
									<td align="right" valign="top"><b>Valor Curso:</b></td>
									<td colspan="2" align="left">R$ <?php print($_ClassDinheiro->formataMoeda($dadosCurso->valor));?></td>
								</tr>
								<tr>
									<td align="right" valign="top" width="15%"><b>Adicional:</b></td>
									<td align='left'><input type="checkbox" name="formapagamento[]" value="adicional" onclick="disen(document.formMatricula.vl_adicional)" <?php print(((is_array($_REQUEST["formapagamento"]) && array_search("adicional", $_REQUEST["formapagamento"]) !== false)?"checked":""));?>></td>
									<td width='85%' align='left'>Valor:&nbsp;<input type="text" name="vl_adicional" onKeydown="Formata(this,20,event,2)" size="20" value="<?php echo $_REQUEST["vl_adicional"]?>" style="text-align:right;" <?php print(((is_array($_REQUEST["formapagamento"]) && array_search("adicional", $_REQUEST["formapagamento"]) !== false)?"":"disabled"));?>></td>
								</tr>
								<tr>
									<td align="right" valign="top" width="15%"><b>Desconto:</b></td>
									<td align='left'><input type="checkbox" name="formapagamento[]" value="desconto" onclick="disen(document.formMatricula.vl_desconto)" <?php print(((is_array($_REQUEST["formapagamento"]) && array_search("desconto", $_REQUEST["formapagamento"]) !== false)?"checked":""));?>></td>
									<td width='85%' align='left'>Valor:&nbsp;<input type="text" name="vl_desconto" onKeydown="Formata(this,20,event,2)" size="20" value="<?php echo $_REQUEST["vl_desconto"]?>" style="text-align:right;" <?php print(((is_array($_REQUEST["formapagamento"]) && array_search("desconto", $_REQUEST["formapagamento"]) !== false)?"":"disabled"));?>></td>
								</tr>
								<tr>
									<td align="right" valign="top" width="15%"><b>Dinheiro:</b></td>
									<td align='left'><input type="checkbox" name="formapagamento[]" value="dinheiro" onclick="disen(document.formMatricula.vl_dinheiro)" <?php print(((is_array($_REQUEST["formapagamento"]) && array_search("dinheiro", $_REQUEST["formapagamento"]) !== false)?"checked":""));?>></td>
									<td width='85%' align='left'>Valor:&nbsp;<input type="text" name="vl_dinheiro" onKeydown="Formata(this,20,event,2)" size="20" value="<?php echo $_REQUEST["vl_dinheiro"]?>" style="text-align:right;" <?php print(((is_array($_REQUEST["formapagamento"]) && array_search("dinheiro", $_REQUEST["formapagamento"]) !== false)?"":"disabled"));?>></td>
								</tr>
								<tr>
									<td align="right" valign="top"><b>Cartão Crédito:</b></td>
									<td align='left'><input type="checkbox" name="formapagamento[]" value="cc" onclick="disen(document.formMatricula.vl_cc)" <?php print(((is_array($_REQUEST["formapagamento"]) && array_search("cc", $_REQUEST["formapagamento"]) !== false)?"checked":""));?>></td>
									<td align='left'>Valor:&nbsp;<input type="text" name="vl_cc" onKeydown="Formata(this,20,event,2)" size="20" value="<?php echo $_REQUEST["vl_cc"]?>" style="text-align:right;" <?php print(((is_array($_REQUEST["formapagamento"]) && array_search("cc", $_REQUEST["formapagamento"]) !== false)?"":"disabled"));?>></td>
								</tr>
								<tr>
									<td align="right" valign="top"><b>Cartão Débito:</b></td>
									<td align='left'><input type="checkbox" name="formapagamento[]" value="cd" onclick="disen(document.formMatricula.vl_cd)" <?php print(((is_array($_REQUEST["formapagamento"]) && array_search("cd", $_REQUEST["formapagamento"]) !== false)?"checked":""));?>></td>
									<td align='left'>Valor:&nbsp;<input type="text" name="vl_cd" onKeydown="Formata(this,20,event,2)" size="20" value="<?php echo $_REQUEST["vl_cd"]?>" style="text-align:right;" <?php print(((is_array($_REQUEST["formapagamento"]) && array_search("cd", $_REQUEST["formapagamento"]) !== false)?"":"disabled"));?>></td>
								</tr>
								<tr>
									...
									<td align='left'><input type="checkbox" name="formapagamento[]" value="bb" onclick="disen(document.formMatricula.vl_bb);
																										   disen(document.formMatricula.parcelas_bb);" <?php print(((is_array($_REQUEST["formapagamento"]) && array_search("bb", $_REQUEST["formapagamento"]) !== false)?"checked":""));?>></td>
									<td align='left'>
										Valor:&nbsp;<input type="text" name="vl_bb" onKeydown="Formata(this,20,event,2)" size="20" value="<?php echo $_REQUEST["vl_bb"]?>" style="text-align:right;" <?php print(((is_array($_REQUEST["formapagamento"]) && array_search("bb", $_REQUEST["formapagamento"]) !== false)?"":"disabled"));?>>
										Parcelas:&nbsp;
										<select name="parcelas_bb" <?php print(((is_array($_REQUEST["formapagamento"]) && array_search("bb", $_REQUEST["formapagamento"]) !== false)?"":"disabled"));?>>
											<option value="1" <?php print((($_REQUEST["parcelas_bb"] == "1")?"selected":""));?>>1</option>
											<option value="2" <?php print((($_REQUEST["parcelas_bb"] == "2")?"selected":""));?>>2</option>
											<option value="3" <?php print((($_REQUEST["parcelas_bb"] == "3")?"selected":""));?>>3</option>
											<option value="4" <?php print((($_REQUEST["parcelas_bb"] == "4")?"selected":""));?>>4</option>
											<option value="5" <?php print((($_REQUEST["parcelas_bb"] == "5")?"selected":""));?>>5</option>
										</select>
									</td>
								</tr>
								<?php
											...
											<tr>
												...
												<td align='left'>
													<table border="0" cellpadding="2" cellspacing="2" align="left">
														...
														<?php
														// Valor da Parcela
														$valorparcela = (($_ClassDinheiro->limpaFormatacaoMoeda($_REQUEST["vl_bb"]) > 0)?($_ClassDinheiro->limpaFormatacaoMoeda($_REQUEST["vl_bb"]) / $_REQUEST["parcelas_bb"]):"");
								...
								<tr>
									...
									<td align='left'><input type="checkbox" name="formapagamento[]" value="ch" onclick="disen(document.formMatricula.vl_ch);
																										 ...
																										 disen(document.formMatricula.nch);" <?php print(((is_array($_REQUEST["formapagamento"]) && array_search("ch", $_REQUEST["formapagamento"]) !== false)?"checked":""));?>></td>
									<td align='left'>
										Valor:&nbsp;<input type="text" name="vl_ch" onKeydown="Formata(this,20,event,2)" size="20" value="<?php echo $_REQUEST["vl_ch"]?>" style="text-align:right;" <?php print(((is_array($_REQUEST["formapagamento"]) && array_search("ch", $_REQUEST["formapagamento"]) !== false)?"":"disabled"));?>>
										Parcelas:&nbsp;
										<select name="parcelas_ch" <?php print(((is_array($_REQUEST["formapagamento"]) && array_search("ch", $_REQUEST["formapagamento"]) !== false)?"":"disabled"));?>>
											<option value="1" <?php print((($_REQUEST["parcelas_ch"] == "1")?"selected":""));?>>1</option>
											<option value="2" <?php print((($_REQUEST["parcelas_ch"] == "2")?"selected":""));?>>2</option>
											<option value="3" <?php print((($_REQUEST["parcelas_ch"] == "3")?"selected":""));?>>3</option>
											<option value="4" <?php print((($_REQUEST["parcelas_ch"] == "4")?"selected":""));?>>4</option>
											<option value="5" <?php print((($_REQUEST["parcelas_ch"] == "5")?"selected":""));?>>5</option>
										</select>
										Bco.:&nbsp;
										<input type="text" size="15" name="bco" value="<?php print($_REQUEST["bco"]);?>" <?php print(((is_array($_REQUEST["formapagamento"]) && array_search("ch", $_REQUEST["formapagamento"]) !== false)?"":"disabled"));?>>
										N. Ch.:&nbsp;
										<input type="text" size="15" name="nch" value="<?php print($_REQUEST["nch"]);?>" <?php print(((is_array($_REQUEST["formapagamento"]) && array_search("ch", $_REQUEST["formapagamento"]) !== false)?"":"disabled"));?>>
									</td>
								</tr>
								<?php
								// Verifica Acão
											...
											<tr>
												...
												<td align='left'>
													<table border="0" cellpadding="2" cellspacing="2" align="left">
														...
														<?php
														// Valor da Parcela
														$valorparcela = (($_ClassDinheiro->limpaFormatacaoMoeda($_REQUEST["vl_ch"]) > 0)?($_ClassDinheiro->limpaFormatacaoMoeda($_REQUEST["vl_ch"]) / $_REQUEST["parcelas_ch"]):"");
														...
														// Lê Parcelas
														for($pp = 1; $pp <= $_REQUEST["parcelas_ch"]; $pp++){
								...
								<tr>
									...
									<td colspan="2"><b>Total Pago:</b>&nbsp;R$ <?php print($_ClassDinheiro->formataMoeda($_ClassDinheiro->limpaFormatacaoMoeda($_REQUEST["vl_dinheiro"])+
																						  								 $_ClassDinheiro->limpaFormatacaoMoeda($_REQUEST["vl_cc"])+
																						  								 $_ClassDinheiro->limpaFormatacaoMoeda($_REQUEST["vl_cd"])+
																						  								 $_ClassDinheiro->limpaFormatacaoMoeda($_REQUEST["vl_bb"])+
																						  								 $_ClassDinheiro->limpaFormatacaoMoeda($_REQUEST["vl_ch"])));?></td>
								</tr>
								<tr>
									<td colspan="3">
										<br>
				...
				// Redieciona
				print($_ClassUtilitarios->redirecionarJS("?sessao=matriculas&ref=novo&etapa=4", 1, array("Este aluno já participa desta turma.")));
		...
		// Redieciona
		print($_ClassUtilitarios->redirecionarJS("?sessao=matriculas&ref=novo&etapa=4", 1, array("É preciso escolher uma turma.")));
