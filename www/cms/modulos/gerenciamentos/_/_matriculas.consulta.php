...
if($_REQUEST["ref"] == ""){
	...
	<tr>
		<td class="table_main">
			<form action="?sessao=matriculas&ref=buscar" method="POST" name="formMatricula">
	...
	<tr>
		<td class="table_main">
			<table width="99%" border="0" cellpadding="2" cellspacing="2">
				...
				if($_SESSION["consultaMatriculasPrivadas"]["todas"] == "sim"){
					?>
					<tr>
				...
				if($_SESSION["consultaMatriculasPrivadas"]["habilitarPalavraChave"] == "sim"){
					?>
					<tr>
						<td align='left'><ol><strong>Palavra-Chave:</strong>&nbsp;<?php echo $_SESSION["consultaMatriculasPrivadas"]["palavraChave"]?></ol></td>
					</tr>
					<?php
				}
				
				// Verifica se foi habilitado o campo de turma
				if($_SESSION["consultaMatriculasPrivadas"]["habilitarTurma"] == "sim"){
					
					// Dados da Turma
					$dadosTurma = $_ClassRn->getDadosTable("turmas", "curso, numero", "id = '" . $_SESSION["consultaMatriculasPrivadas"]["turma"] . "'");
							...
							<table class="consulta" cellspacing="1" align="center">
								...
								<tbody>
									...
									if($_SESSION["consultaMatriculasPrivadas"]["todas"] != "sim"){
										...
										$sql .= (($_SESSION["consultaMatriculasPrivadas"]["habilitarPalavraChave"] == "sim")?" matriculas.empresa <= 0 AND alunos.id = matriculas.aluno AND alunos.rg LIKE '%" . $_SESSION["consultaMatriculasPrivadas"]["palavraChave"] . "%' AND alunos.deletado = 'N' AND matriculas.deletado = 'N' OR
																										     matriculas.empresa <= 0 AND alunos.id = matriculas.aluno AND alunos.cpf LIKE '%" . $_ClassUtilitarios->tiraMask($_SESSION["consultaMatriculasPrivadas"]["palavraChave"]) . "%' AND alunos.deletado = 'N' AND matriculas.deletado = 'N' OR
																										 	 matriculas.empresa <= 0 AND alunos.id = matriculas.aluno AND alunos.nome LIKE '%" . $_SESSION["consultaMatriculasPrivadas"]["palavraChave"] . "%' AND alunos.deletado = 'N' AND matriculas.deletado = 'N' AND ":"");
										$sql .= (($_SESSION["consultaMatriculasPrivadas"]["habilitarTurma"] == "sim")?" matriculas.empresa <= 0 AND alunos.id = matriculas.aluno AND matriculas.turma = '" . $_SESSION["consultaMatriculasPrivadas"]["turma"] . "' AND matriculas.deletado = 'N' AND alunos.deletado = 'N' AND ":"");
									...
									$_ClassPaginacao->setUrl("?sessao=matriculas&ref=buscar&ordem=" . $_REQUEST["ordem"] . "&campo=" . $_REQUEST["campo"] . "&filtro=" . $_REQUEST["filtro"]);
									...
									if($_ClassPaginacao->getTotalAchadoQuery() == 0){
										?>
										<tr>
											<td align="center" colspan="6"><b>Nenhum resultado encontrado.</b></td>
										</tr>
										<?
									}else{
										...
										while($trazResultados = mysql_fetch_object($_ClassPaginacao->getBusca())){
											...
											<tr class=row0>
												...
												<td align="center"><a name="<?=$trazResultados->id?>"></a><a href="?sessao=matriculas<?="&pg=" . $_REQUEST['pg'] . "&ref=edit&idRegistro=" . $trazResultados->id?>"><?php print($dadosAluno->nome);?></a></td>
