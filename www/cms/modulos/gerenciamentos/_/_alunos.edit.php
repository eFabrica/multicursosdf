...
if($_REQUEST["act"] == "salvar"){
	...
	if($_ClassMensagens->getMensagem_erro() == ""){
		...
		if($_REQUEST["cpf"] != ""){
			...
			// Busca Cpf no banco
			$buscaAlunos = $_ClassMysql->query("SELECT * FROM `alunos` WHERE id != '" . $_REQUEST["idRegistro"] . "' AND deletado = 'N' AND cpf = '" . $_ClassUtilitarios->deixaN($_REQUEST["cpf"]) . "'");
	...
	if($_ClassMensagens->getMensagem_erro() == ""){
		...
		if($editaAluno){
			...
			// Redireciona
			print($_ClassUtilitarios->redirecionarJS("?sessao=alunos&ref=sucesso&idAluno=" . $_REQUEST["idRegistro"]));
...
if(!$sucesso){
	...
	// Dados do Aluno
	$dadosAluno = $_ClassRn->getDadosTable("alunos", "*", "id = '" . $_REQUEST["idRegistro"] . "'");
			...
			<form action="" method="POST" name="formAluno">
				...
				<table width="100%" border="0" cellpadding="2" cellspacing="2" align="left">
					...
					<tr>
						...
						<td align="left"><input type="text" name="dataform" size="12" maxlength="10" onKeyUp="maskData(this, document.formAluno.cpf)" value="<?php print((($_REQUEST["dataform"] != "")?$_REQUEST["dataform"]:$_ClassData->transformaData($dadosAluno->dataform, 2)));?>"></td>
					</tr>
					<tr>
					...
					<tr>
						...
						<td align="left">
							<select name="orgao">
                                ...
                                <option value="RN" <?php echo ((($_REQUEST["orgao"] != "")?$_REQUEST["orgao"]:$dadosAluno->orgao) == "RN")?"selected":""?>>RN</option>
