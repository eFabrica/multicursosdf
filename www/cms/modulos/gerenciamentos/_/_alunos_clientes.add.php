...
if($_REQUEST["act"] == "salvar"){
	...
	if($_ClassMensagens->getMensagem_erro() == ""){
		...
		$dadosAluno = $_ClassRn->getDadosTable("alunos", "id", "cpf = '" . $_ClassUtilitarios->deixaN($_REQUEST["cpf"]) . "' AND empresa != '" . $_dadosLogado->empresa . "' AND deletado = 'N'");
		...
		if($_ClassRn->getTot() > 0){
			...
			// Redireciona
			print($_ClassUtilitarios->redirecionarJS("?sessao=alunos_clientes&ref=sucesso&idAluno=" . mysql_insert_id()));
			
		}else{
		
			// Verifica se já existe este aluno
			$_ClassRn->getDadosTable("alunos", "id", "cpf = '" . $_ClassUtilitarios->deixaN($_REQUEST["cpf"]) . "' AND empresa = '" . $_dadosLogado->empresa . "' AND deletado = 'N'");
			...
			<form action="" method="POST" name="formAluno">
				...
				<table width="100%" border="0" cellpadding="2" cellspacing="2" align="left">
					...
					<tr>
						...
						<td align="left"><input type="text" name="datanascimento" size="12" maxlength="10" onKeyUp="maskData(this, document.formAluno.rg)" value="<?php echo $_REQUEST["datanascimento"]?>"></td>
					</tr>
					<tr>
					...
					<tr>
						...
						<td align="left"><input type="text" name="cpf" size="18" maxlength="14" onKeyUp="maskCPF(this, this)" value="<?php echo $_REQUEST["cpf"]?>"></td>
					</tr>
					<tr>
						<td colspan="2">
							<br>
