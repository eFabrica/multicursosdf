...
if($dadosMatricula->aluno > 0){
	...
	if($_REQUEST["act"] == "salvar"){
		...
		if($_ClassMensagens->getMensagem_erro() == ""){
			...
			if($_REQUEST["cpf"] != ""){
				...
				// Busca Cpf no banco
				$buscaAlunos = $_ClassMysql->query("SELECT * FROM `alunos` WHERE id != '" . $dadosMatricula->aluno . "' AND deletado = 'N' AND cpf = '" . $_ClassUtilitarios->deixaN($_REQUEST["cpf"]) . "'");
		...
		if($_ClassMensagens->getMensagem_erro() == ""){
			...
			$editaAluno = $_ClassMysql->query("UPDATE `alunos` SET numerorg = '" . $_REQUEST["numerorg"] . "',
															  	   ...
															  	   ultimoeditou = '" . $_dadosLogado->id . "',
								   							  	   datahorae = now() WHERE id = '" . $dadosMatricula->aluno . "'");
			
			// Verifica se Editou
			if($editaAluno){
				
				// Redireciona
				print($_ClassUtilitarios->redirecionarJS("?sessao=matriculas&ref=edit&etapa=2"));
				...
				<table width="100%" border="0" cellpadding="2" cellspacing="2" align="left">
					...
					<tr>
				 	...
				 	<tr>
						...
						<td align='left'><input type="text" name="datanascimento" size="12" maxlength="10" onKeyUp="maskData(this, document.formMatricula.cpf)" value="<?php echo (($_REQUEST["datanascimento"] != "")?$_REQUEST["datanascimento"]:$_ClassData->transformaData($dadosAluno->datanascimento, 2));?>"></td>
					</tr>
					<tr>
						<td align="right"><b>CPF:</b></td>
						<td align='left'><input type="text" name="cpf" size="18" maxlength="14" onKeyUp="maskCPF(this, document.formMatricula.rg)" value="<?php echo (($_REQUEST["cpf"] != "")?$_REQUEST["cpf"]:$_ClassUtilitarios->formataCPF($dadosAluno->cpf));?>"></td>
					</tr>
					<tr>
					...
					<tr>
						...
						<td align='left'><input type="text" name="dataexp" size="12" maxlength="10" onKeyUp="maskData(this, document.formMatricula.sexo)" value="<?php echo (($_REQUEST["dataexp"] != "")?$_REQUEST["dataexp"]:$_ClassData->transformaData($dadosAluno->dataexp, 2));?>"></td>
					</tr>
				 	<tr>
	...
	// Redieciona
	print($_ClassUtilitarios->redirecionarJS("?sessao=matriculas" . $_SESSION["consultaMatricula"]["urlPesquisa"], 1, array("é preciso selecionar uma matr�cula.")));
