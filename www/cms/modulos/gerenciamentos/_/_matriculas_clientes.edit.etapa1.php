	...
	if($_REQUEST["act"] == "salvar"){
		...
		if($_ClassMensagens->getMensagem_erro() == ""){
			...
			if($_REQUEST["cpf"] != ""){
				...
				$_ClassRn->getDadosTable("alunos", "id", "id != '" . $dadosMatricula->aluno . "' AND cpf = '" . $_ClassUtilitarios->deixaN($_REQUEST["cpf"]) . "' AND deletado = 'N'");
		...
		if($_ClassMensagens->getMensagem_erro() == ""){
			...
			if($editaAluno){
				...
				// Redireciona
				print($_ClassUtilitarios->redirecionarJS("?sessao=matriculas&subsessao=" . $_REQUEST["subsessao"] . "&ref=edit&etapa=2"));
