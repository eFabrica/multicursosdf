...
if($dadosMatricula->aluno > 0){
			...
			if($dadosTurma->vagasocupadas < $dadosTurma->vagas || $dadosMatricula->turma == $dadosTurma->id){
				...
				while($trazMatriculas = mysql_fetch_object($buscaMatriculas)){
					...
					if($dadosAluno->cpf == $dadosAlunoMat->cpf && $permissao == true){
						...
						// Redieciona
						print($_ClassUtilitarios->redirecionarJS("?sessao=matriculas&subsessao=" . $_REQUEST["subsessao"] . "&ref=edit&etapa=2&curso=" . $_REQUEST["curso"], 1, array("Este aluno já está nesta turma.")));
				...
				*/
				...
				if($permissao){
					...
					if($editaMatricula){
						...
						# Sucesso
							...
							$_SESSION["matricula"]["idMatricula"] = $dadosMatricula->id;
							
							// Redireciona
							print($_ClassUtilitarios->redirecionarJS("?sessao=matriculas&subsessao=" . $_REQUEST["subsessao"] . "&ref=edit&etapa=4"));
				...
				// Redieciona
				print($_ClassUtilitarios->redirecionarJS("?sessao=matriculas&subsessao=" . $_REQUEST["subsessao"] . "&ref=edit&etapa=3", 1, array("Turma selecionada não tem vagas.")));
	...
	<tr>
		<td class="table_main">
			<form action="?sessao=matriculas&subsessao=<?php print($_REQUEST["subsessao"]); ?>&ref=edit&etapa=3&submenu=buscar" method="POST" name="formMatricula_Busca">
	...
	// Redieciona
	print($_ClassUtilitarios->redirecionarJS("?sessao=matriculas&subsessao=" . $_REQUEST["subsessao"] . $_SESSION["consultaMatricula"]["urlPesquisa"], 1, array("É preciso selecionar uma matrícula.")));
