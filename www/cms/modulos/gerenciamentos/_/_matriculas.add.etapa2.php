	...
	if($_REQUEST["act"] == "salvar"){
		...
		if($_ClassMensagens->getMensagem_erro() == ""){
			...
			if($_REQUEST["cpf"] != ""){
				...
				// Busca Cpf no banco
				$buscaAlunos = $_ClassMysql->query("SELECT * FROM `alunos` WHERE id != '" . $_SESSION["matricula"]["idAluno"] . "' AND deletado = 'N' AND cpf = '" . $_ClassUtilitarios->deixaN($_REQUEST["cpf"]) . "'");
		...
		if($_ClassMensagens->getMensagem_erro() == ""){
			...
			if($editaAluno){
				...
				// Redireciona
				print($_ClassUtilitarios->redirecionarJS("?sessao=matriculas&ref=novo&etapa=3"));
