...
if($_SESSION["matricula"]["idAluno"] > 0){
	...
	if($_REQUEST["act"] == "salvar"){
		...
		// Redireciona
		print($_ClassUtilitarios->redirecionarJS("?sessao=matriculas&subsessao=" . $_REQUEST["subsessao"] . "&ref=novo&etapa=5"));
	...
	if(!$sucesso){
		...
		// Dados da empresa
		$dadosEmpresa = $_ClassRn->getDadosTable("clientes", "razaosocial, cnpj", "id = '" . $_SESSION["matricula"]["idEmpresa"] . "'");
		?>
		<tr>
			...
			<td class="table_main">
				<table width="100%" border="0" cellpadding="2" cellspacing="2" align="left">
					...
					<tr>
						...
						<td align='left'><?php print($dadosEmpresa->razaosocial . " (" . $_ClassUtilitarios->formataCNPJ($dadosEmpresa->cnpj) . ")");?></td>
					</tr>
				</table>
			</td>
		</tr>
		...
		<tr>
			<td class="table_main">
				<form action="" method="POST" name="formMatricula">
					...
					<input type="hidden" name="subsessao" value="<?php print($_REQUEST["subsessao"]);?>">
					<table width="100%" border="0" cellpadding="2" cellspacing="2" align="left">
						<tr>
