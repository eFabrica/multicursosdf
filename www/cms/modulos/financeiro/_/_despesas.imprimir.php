...
if($_SESSION["consultaDespesas"]["todas"] != "sim"){
	...
	$sql .= (($_SESSION["consultaDespesas"]["habilitarData"] == "sim")?" datahorae >= '" . $_ClassData->transformaData($_SESSION["consultaDespesas"]["dataI"]) . " 00:00:00' AND datahorae <= '" . $_ClassData->transformaData($_SESSION["consultaDespesas"]["dataF"]) . " 23:59:59' AND":"");
...
// Busca Despesas
$buscaDespesas = $_ClassMysql->query($sql);

// Calcula Número de Paginas
$totPag = ceil(mysql_num_rows($buscaDespesas)/35);
?>
<html>
	<head>
	...
	<body>
		...
		for($i = 1; $i <= $totPag; $i++){
			...
			<table width="99%" border="0" cellpadding="2" cellspacing="2">
				<tr>
					<td align="center">
						<h2><u>Relatório de Despesas</u></h2>
			...
			<table width="100%" border="0" cellspacing="2" cellpadding="2">
				...
				// Busca Despesas
				$buscaDespesas = $_ClassMysql->query($sql . " LIMIT " . $inicioLimit . "," . 35);
				
				// Traz Despesas
				while($trazDespesas = mysql_fetch_object($buscaDespesas)){
					
					// Total Da Página
					$totalPag += $trazDespesas->valor;
					
					// Dados do Usuário
					$dadosUsuario = $_ClassRn->getDadosTable("usuarios", "*", "id = '" . $trazDespesas->ultimoeditou . "'");
					?>
					<tr>
						<td style="border-bottom:1px solid #333333;border-right:1px solid #333333;""><?php print(strtoupper(substr($trazDespesas->descricao, 0, 50)));?></td>
						<td align="right" style="border-bottom:1px solid #333333;border-right:1px solid #333333;">R$ <?php print($_ClassDinheiro->formataMoeda($trazDespesas->valor));?></td>
						<td align="center" style="border-bottom:1px solid #333333;border-right:1px solid #333333;"><?php print(substr($_ClassData->transformaData($trazDespesas->datahorae, 3), 0, 10));?></td>
						<td align="center" style="border-bottom:1px solid #333333;border-right:1px solid #333333;"><?php print(substr($_ClassData->transformaData($trazDespesas->datahorae, 3), 11, 8));?></td>
